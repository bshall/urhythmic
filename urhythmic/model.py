...
class UrhythmicGlobal(nn.Module):
    """Urhythmic (Global), a voice and rhythm conversion system that does not require text or parallel data.

    Args:
        segmenter (Segmenter): the segmentation and clustering block groups similar units into short segments.
            The segments are then combined into coarser groups approximating sonorants, obstruents, and silences.
        rhythm_model (RhythmModelGlobal): the rhythm modeling block estimates speaking rate.
        time_stretcher (TimeStretcherGlobal): the time-stretching block down/up-samples the speech units to match the target speaking rate.
        vocoder (HifiganGenerator): the vocoder converts the speech units into an audio waveform.
    """
    ...
    @torch.inference_mode()
    def forward(self, units: torch.Tensor, log_probs: torch.Tensor):
        """Convert the to the target speaker's voice and rhythm
